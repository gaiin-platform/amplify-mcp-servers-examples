"""
AWS Lambda handler for Image Processing MCP Server
...
from image_mcp_server.server import app
from image_mcp_server import server as server_module
from image_mcp_server.image_manager import ImageManager

# S3 bucket for persistent file storage
S3_BUCKET = 'jupyter-mcp-workspaces-654654422653'

# Initialize image manager globally for warm starts
if server_module.image_manager is None:
    print("Initializing Image Manager (cold start)...")
    server_module.image_manager = ImageManager(
        working_dir=os.environ.get('IMAGE_WORKING_DIR', '/tmp/images'),
        s3_bucket=S3_BUCKET,
        enable_s3=True
    )
    print("Image Manager initialized successfully")
else:
    print("Using existing Image Manager (warm start)")
