"""
AWS Lambda handler for Node.js Execution MCP Server
...
from nodejs_mcp_server.server import app
from nodejs_mcp_server import server as server_module
from nodejs_mcp_server.execution_manager import ExecutionManager

# S3 bucket for persistent file storage
S3_BUCKET = 'jupyter-mcp-workspaces-654654422653'

# Initialize execution manager globally for warm starts
if server_module.execution_manager is None:
    print("Initializing Execution Manager (cold start)...")
    server_module.execution_manager = ExecutionManager(
        working_dir=os.environ.get('NODEJS_WORKING_DIR', '/tmp/nodejs'),
        s3_bucket=S3_BUCKET,
        enable_s3=True
    )
    print("Execution Manager initialized successfully")
else:
    print("Using existing Execution Manager (warm start)")
